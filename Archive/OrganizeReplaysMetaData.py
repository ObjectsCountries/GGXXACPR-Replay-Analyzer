...
def CreatePath(file_path: str) -> None:
    """
...
def MoveFile(temp_path: str, file: str) -> None:
    """
    Handles duplicate file cases.
    """
...
def ParseMetadata(replay_file_path):
    ...
    for label, data in metadata_dictionary.items():
        ...
        if data[1] != 'c':
            parsedDict[label] = int.from_bytes(
...
if len(sys.argv) == 2:  # this statement makes the 'reformat' parameter optional
    ...
    if reformat == 'reformat':
        # this command will move all replay files to the root folder, and delete the other folders.
        MoveToRootFolder(file_path, file_path)
else:
    config_dictionary = {}
        ...
        if not tempBool:
            ...
            if (steamID == -1):
                ...
                print(f'steamID saved as: {
                      steamID}, if all of your matches are being moved into \'spectated matches\', you may need to edit the config file to correct a mistake.')
    ...
    for file in replay_files:
        if 'ggr' in file.lower():  # ensure its a replay file
            ...
            # if it was a spectated match, move it into spectated matches.-------------------------------------------
            if player == '':
                ...
                if not CheckConfDict(config_dictionary, str(metaData[opponent+' steam id'])):
                    ...
                    config_dictionary[str(
                        metaData[opponent+' steam id'])] = opName

                # organization--------------------------------------------------------------------------------------------------------
                temp_path = file_path+"\\"+"As "+metaData[player+' char']+'\\'+"Against "+metaData[opponent +
                                                                                                   ' char']+'\\'+"Against "+config_dictionary[str(metaData[opponent+' steam id'])]
                ...
                with open('replayOrganizerConfig.ini', 'w', encoding="utf-8") as file:
                    for steamID in config_dictionary:
                        file.write(str(steamID)+"=" +
                                   config_dictionary[steamID]+"\n")
